 ...
 */
export class Fifo implements Disposable, NativeWrapper<NativeFifo> {
  private native: NativeFifo;
   ...
   */
  alloc(nbElems: number, elemSize: number, flags: AVFifoFlag = AVFLAG_NONE): void {
   ...
   */
  async write(buf: Buffer, nbElems: number): Promise<number> {
   ...
   */
  async read(buf: Buffer, nbElems: number): Promise<number> {
   ...
   */
  async peek(buf: Buffer, nbElems: number, offset = 0): Promise<number> {
   ...
   */
  peekSync(buf: Buffer, nbElems: number, offset = 0): number {
